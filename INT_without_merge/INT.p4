...
control SwitchEgress(
    ...
    action set_max_number_hop_t() {
        eg_md.max_number = eg_md.priority_hop_t;    // stage 4
        eg_md.mapinfo_1 = 1;
        eg_md.md = eg_md.hop_t;    //stage 4
    }

    action set_max_number_bd() {
        eg_md.max_number = hdr.bridge.priority_bd;    //stage 4
        eg_md.mapinfo_1 = 2;    //stage 4
        eg_md.md = eg_md.bytes;    //stage 4
    }

    action set_max_number_eport() {
        eg_md.max_number_1 = eg_md.priority_outport;    // stage 4
        eg_md.mapinfo_2 = 3;    // stage 4
        eg_md.md_1 = hdr.bridge.eport;    // stage 4
    }

    action set_max_number_port() {
        eg_md.max_number_1 = eg_md.priority_inport;    // stage 4
        eg_md.mapinfo_2 = 4;    // stage 4
        eg_md.md_1 = hdr.bridge.port;    // stage 4
    }

    action set_max_num() {
        eg_md.max_num = eg_md.max_number_1;    //stage 7
        eg_md.mapinfo = eg_md.mapinfo_2;    //stage 7
        eg_md.metadata = (bit<16>) eg_md.md_1;    //stage 7 and stage 8
    }

    action set_max_num_1() {
        eg_md.max_num = eg_md.max_number;    //stage 7
        eg_md.mapinfo = eg_md.mapinfo_1;    //stage 7
        eg_md.metadata = eg_md.md;    //stage 7
    }
    ...
    apply {
        ...
        hop_t_priority.apply();    //stage 1
        bd_priority.apply();    //stage 2
        sub_priority();    //stage 3

        if (eg_md.diff >= 0){    // stage 4
            set_max_number_hop_t();
        }else{
            set_max_number_bd();
        }           

        if (eg_md.diff_1 >= 0){  //stage 4
            set_max_number_eport();
        }else{
            set_max_number_port();
        }
      
        sub_max_number();    //stage 6
        if (eg_md.diff >= 0){  //stage 7
            set_max_num();
        } else{
            set_max_num_1();
        }

        if (hdr.bridge.first_pkt == 8w0x1){  //stage 8
            eg_md.temp = dbt_priority_init.execute(hdr.bridge.index);  //stage8
        }else{
            eg_md.temp = dbt_priority_action.execute(hdr.bridge.index);    //stage 8
        }

        eg_md.final_map = dbt_mapinfo_action.execute(hdr.bridge.index);    //stage 10
        eg_md.final_md = dbt_metadata_action.execute(hdr.bridge.index);    //stage 10

        if (hdr.bridge.if_egress == 8w0x1){    //stage 10
	    ...
	    add_md();
                 ...
                 hdr.int_header.setInvalid();    //stage 10
                 hdr.ipv4.protocol = TYPE_TCP;    //stage 10
            }
        }
    hdr.bridge.setInvalid();  //stage 0
    }
}
